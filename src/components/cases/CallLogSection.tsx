    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        ...
        {showNotes && (
          <div className="space-y-2">
            <Textarea
              placeholder="Nota opcional sobre a ligação..."
              value={notes}
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-gray-400 text-center py-3">Nenhuma ligação registrada ainda.</p>
      ) : (
        <div className="space-y-2">
          {visibleLogs.map((log) => (
            <div key={log.id} className="flex items-start gap-2 p-2.5 bg-gray-50 rounded-lg">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-gray-700">
                  {format(new Date(log.logged_at), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR })}
                </p>
                {log.notes && (
          ...
          {logs.length > 3 && (
            ...
            >
              {expanded ? (
                ...
                <><ChevronDown className="w-3 h-3" /> Ver mais {logs.length - 3} registros</>
              )}
            </button>
          )}
        </div>
